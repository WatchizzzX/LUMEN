...
[ExecuteAlways]
public class CableProceduralSimple : BaseCableRenderer
{
    LineRenderer line;
    ...
    void Start()
    {
        ...
        {
            Debug.LogError(
                "No Endpoint Transform assigned to Cable_Procedural component attached to " + gameObject.name);
            return;
        }
    ...
    void Draw()
    {
        ...
        while (i < pointsInLineRenderer)
        {
            ...
            Vector3 swayAtPoint = swayMultiplier * transform.TransformDirection(new Vector3(
                Mathf.Sin(swayValue) * swayXMultiplier, Mathf.Cos(2 * swayValue + Mathf.PI) * .5f * swayYMultiplier,
                0));
    ...
    public override void RenderCable()
    {
        OnValidate();
    }
}